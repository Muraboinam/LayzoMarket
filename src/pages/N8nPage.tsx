...
import { useN8nWorkflowTemplates } from '../hooks/useFirestoreTemplates';
...
const N8nPage: React.FC = () => {
  ...
  // Use Firestore hook to fetch n8n workflow templates
  const { templates: firestoreTemplates, loading: templatesLoading, error: templatesError } = useN8nWorkflowTemplates();
  ...
  const n8nProducts = firestoreTemplates.length > 0 ? firestoreTemplates : [];

  const categories = [
    { id: 'all', name: 'All Workflows', count: 6 },
    { id: 'customer-support', name: 'Customer Support', count: 15 },
    { id: 'content-creation', name: 'Content Creation', count: 12 },
    { id: 'data-analysis', name: 'Data Analysis', count: 18 },
    { id: 'lead-generation', name: 'Lead Generation', count: 10 },
    { id: 'social-media', name: 'Social Media', count: 14 },
    { id: 'sales-automation', name: 'Sales Automation', count: 16 }
  ];

  // Filter and sort products
  const filteredProducts = useMemo(() => {
    let filtered = n8nProducts.filter(product => {
    ...
    return filtered;
  }, [searchQuery, selectedCategory, priceRange, n8nProducts]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900">
      ...
      <div className="pt-24 pb-12 text-center">
        <div className="max-w-4xl mx-auto px-4">
          <h1 className="text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 mb-4">
          ...
          <div className="text-xl text-blue-200 mb-8">
            {templatesLoading ? (
              <div className="flex items-center justify-center space-x-2">
                ...
                <span>Loading n8n workflow templates from Firestore...</span>
              </div>
            ) : templatesError ? (
              ...
              <span>
                Powerful automation workflows and intelligent AI agents to streamline your business processes and boost productivity.
          ...
          <div className="relative max-w-2xl mx-auto">
            ...
            <input
              type="text"
              placeholder="Search workflows and AI agents..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-12 pr-4 py-4 bg-white/10 backdrop-blur-sm border border-blue-300/30 rounded-xl text-white placeholder-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent"
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-16">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-80 space-y-6">
            {/* Filters Header */}
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-blue-300/20">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-white">Filters</h3>
                <button className="text-blue-300 hover:text-white text-sm">Clear All</button>
              </div>
                ...
                <div className="space-y-4">
                  <div className="relative">
                    <input
                      ...
                      className="w-full h-2 bg-blue-700 rounded-lg appearance-none cursor-pointer slider"
                    />
              ...
              <div className="flex items-center space-x-4">
                <div className="flex bg-white/10 rounded-lg p-1">
                  <button
                    ...
                    className={`p-2 rounded ${viewMode === 'grid' ? 'bg-blue-600 text-white' : 'text-blue-300'}`}
                  >
                    <Grid className="w-5 h-5" />
                  </button>
                  <button
                    onClick={() => setViewMode('list')}
                    className={`p-2 rounded ${viewMode === 'list' ? 'bg-blue-600 text-white' : 'text-blue-300'}`}
                  >
