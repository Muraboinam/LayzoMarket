...
import { useAppTemplates } from '../hooks/useFirestoreTemplates';
...
const AppsPage: React.FC = () => {
  ...
  // Use Firestore hook to fetch app templates
  const { templates: firestoreTemplates, loading: templatesLoading, error: templatesError } = useAppTemplates();
  ...
  // Use Firestore templates or fallback to mock data
  const mobileProducts = firestoreTemplates.length > 0 ? firestoreTemplates : [];

  const categories = [
    { id: 'all', name: 'All Apps', count: 6 },
    { id: 'React Native', name: 'React Native', count: 25 },
    { id: 'Flutter', name: 'Flutter', count: 18 },
    { id: 'productivity', name: 'Productivity', count: 14 },
    { id: 'finance', name: 'Finance', count: 20 }
  ];

  // Filter and sort products
  const filteredProducts = useMemo(() => {
    let filtered = mobileProducts.filter(product => {
    ...
    return filtered;
  }, [searchQuery, selectedCategory, priceRange, mobileProducts]);
          ...
          <div className="text-xl text-purple-200 mb-8">
            {templatesLoading ? (
              <div className="flex items-center justify-center space-x-2">
                ...
                <span>Loading app templates from Firestore...</span>
              </div>
            ) : templatesError ? (
              ...
              <span>
                Discover powerful applications and software solutions to boost your productivity and streamline your workflow.
